...
function handleDeviceTypeChange(event) {
    ...
    var assetType = datasources["asset_types"].latestData().find(function(assetType) {
        return assetType.data.device_type == $("#device-types").val();
    });

    datasources["assets"].sendData({
        "query": CB_PORTAL.ClearBlade.Query().equalTo("type", assetType.data.id)
    });
